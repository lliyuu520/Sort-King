...
class McpServer {
  ...
  async handleRpcRequest(request) {
    ...
    if (method === 'prompts/get') {
      ...
      try {
        return this.createResult(request.id, this.promptProvider.getPrompt(
          params.name, params.arguments === undefined ? {} : params.arguments
        ));
      } catch (error) {
        return this.createError(request.id, error.code === -32602 ? -32602 : -32603, error.message);
      }
    }
